...
import { providerCatalog, refreshHostMappings, withPaseoClient } from "@/lib/paseo";
...
export async function POST(_request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const catalog = await withPaseoClient(user.id, id, async (client) => providerCatalog(await client.providers.waitForReady({ timeoutMs: 60_000 })));
