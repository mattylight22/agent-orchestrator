...
export function SettingsPage() {
  ...
  const catalogs = useMemo(() => {
    const ready = new Map<string, (typeof snapshot.providerCatalogs)[string][number]>();
    for (const host of snapshot.hosts) {
      if (!host.enabled) continue;
      for (const model of snapshot.providerCatalogs[host.id] ?? []) {
        if (model.status === "ready") ready.set(`${model.provider}:${model.model}`, model);
      }
    }
    return [...ready.values()];
  }, [snapshot.hosts, snapshot.providerCatalogs]);
...
function PaseoHosts({ hosts, providerCatalogs, request, onDeployAws }: { hosts: PaseoHost[]; providerCatalogs: Record<string, Array<unknown>>; request<T = unknown>(url: string, init?: RequestInit): Promise<T>; onDeployAws(): void }) {
  ...
  return <>
    ...
    {!hosts.length && <div className="empty-connection"><Server/><div><strong>No Agent Instances Connected</strong><small>Add an instance to discover providers, projects, and workspaces.</small></div></div>}
    <div className="add-host-row"><div><Plus/><span><strong>Connect Another Agent Instance</strong><small>Use the guided setup to choose Relay or a direct Tailscale connection.</small></span></div><button className="primary" onClick={() => setWizardOpen(true)}><Plus/>Add Instance</button></div>
    {wizardOpen && <PaseoSetupWizard request={request} onClose={() => setWizardOpen(false)} onDeployAws={() => { setWizardOpen(false); onDeployAws(); }}/>}
  </>;
}

export function PaseoSetupWizard({ request, onClose, onDeployAws }: { request<T = unknown>(url: string, init?: RequestInit): Promise<T>; onClose(): void; onDeployAws?: () => void }) {
  ...
  return <div className="dialog-backdrop" role="presentation"><section className="dialog connection-wizard" role="dialog" aria-modal="true" aria-labelledby="paseo-wizard-title">
    <header><div><span className="eyebrow">Agent Instance</span><h2 id="paseo-wizard-title">Connect an Agent Instance</h2></div><button className="icon-button" aria-label="Close Setup" onClick={onClose}><X/></button></header>
    ...
    {step === 1 && <div className="wizard-panel"><div><h3>How Should Agent God Mode Reach This Instance?</h3><p>Select the connection method you want to use for this Agent Instance.</p></div><div className="connection-methods"><button type="button" className={transport === "tailscale" ? "selected" : ""} onClick={() => setTransport("tailscale")}><span><Wifi/></span><strong>Direct via Tailscale</strong><small>Your browser connects straight to Paseo through Tailscale on this device. Traffic stays inside your tailnet.</small><i>{transport === "tailscale" && <Check/>}</i></button><button type="button" className={transport === "relay" ? "selected" : ""} onClick={() => setTransport("relay")}><span><RadioTower/></span><b className="connection-method-badge">Easiest</b><strong>Paseo Relay</strong><small>Paseo handles an end-to-end encrypted connection to your Agent Instance. It requires only outbound internet access and no inbound ports.</small><i>{transport === "relay" && <Check/>}</i></button>{onDeployAws && <button type="button" onClick={onDeployAws}><span><Cloud/></span><strong>Deploy on AWS</strong><small>Connect an AWS account with OIDC, create an always-on Agent Instance, and pair it automatically through Relay.</small><i><ArrowRight/></i></button>}</div><footer><button className="primary" onClick={() => setStep(2)}>Continue<ArrowRight/></button></footer></div>}
    {step === 2 && <form className="wizard-panel" onSubmit={connect}><div><h3>{transport === "relay" ? "Pair Through Paseo Relay" : "Connect Through Tailscale"}</h3><p>{transport === "relay" ? <>Run <code>paseo daemon pair</code> on the Agent Instance and paste the complete one-time pairing link.</> : <>Make sure Tailscale is connected on this device, then enter the Agent Instance’s secure <code>.ts.net</code> WebSocket endpoint.</>}</p></div>{error && <div className="banner error" role="alert">{error}</div>}<label>Display Name<input value={name} onChange={(event) => setName(event.target.value)} placeholder="Build server" autoFocus required/></label>{transport === "relay" ? <label>Pairing Link<input type="password" autoComplete="off" value={pairingLink} onChange={(event) => setPairingLink(event.target.value)} placeholder="https://app.paseo.sh/#offer=…" required/><small><LockKeyhole/>Your pairing link is stored securely.</small></label> : <label>Tailscale WebSocket Endpoint<input value={endpoint} onChange={(event) => setEndpoint(event.target.value)} placeholder="wss://build-host.your-tailnet.ts.net/ws" spellCheck={false} required/><small><Shield/>Your browser connects directly to this private address; the endpoint is never exposed publicly.</small></label>}<div className="wizard-callout">{transport === "relay" ? <KeyRound/> : <Link2/>}<span><strong>{transport === "relay" ? "Secure Relay Connection" : "Direct Browser Connection"}</strong><small>{transport === "relay" ? "We’ll verify the Agent Instance and load its available providers before saving." : "This browser will verify the daemon, providers, projects, and workspaces over your tailnet."}</small></span></div><footer><button type="button" className="button" onClick={() => { setError(""); setStep(1); }}><ArrowLeft/>Back</button><button className="primary">Connect and Verify<ArrowRight/></button></footer></form>}
    {step === 3 && <div className="wizard-verifying" role="status"><span><LoaderCircle className="spinning"/></span><h3>Verifying {transport === "relay" ? "Relay" : "Tailscale"} Connection…</h3><p>Checking the Agent Instance, loading providers, and matching repositories.</p></div>}
    {step === 4 && <div className="wizard-complete"><span><Check/></span><h3>Agent Instance Connected</h3><p>Agent God Mode found {result?.providerCount ?? 0} provider models and matched {result?.mappingCount ?? 0} repositories. This method is now primary; any other configured method remains available as fallback.</p>{result?.mappingWarning && <div className="banner warning">The connection is saved, but repository matching needs attention: {result.mappingWarning}</div>}<footer><button className="primary" onClick={onClose}>Done</button></footer></div>}
  </section></div>;
}
...
function RoleEditor({ role, config, catalogs, onChange }: { role: AgentRole; config: RoleConfig; catalogs: ProviderModel[]; onChange(config: RoleConfig): void }) {
  const providers = [...new Set(catalogs.map((item) => item.provider))];
  const provider = providers.includes(config.provider) ? config.provider : "";
  const models = catalogs.filter((item) => item.provider === provider);
  const model = models.some((item) => item.model === config.model) ? config.model : "";
  const selected = models.find((item) => item.model === model);
  const label = provider && model ? `${provider}/${model}` : "Choose an Authenticated Provider";
  return <article className="role-card"><header><div className={`role-icon ${role}`}><Bot/></div><div><strong>{role[0].toUpperCase() + role.slice(1)}</strong><code>{label}</code></div></header><div className="field-grid"><label>Provider<select value={provider} onChange={(event) => { const nextProvider = event.target.value; const nextModel = catalogs.find((item) => item.provider === nextProvider); onChange({ provider: nextProvider, model: nextModel?.model ?? "" }); }}><option value="" disabled>Select Provider</option>{providers.map((item) => <option value={item} key={item}>{item}</option>)}</select></label><label>Model<select value={model} disabled={!provider} onChange={(event) => onChange({ ...config, provider, model: event.target.value, modeId: undefined, thinkingOptionId: undefined })}><option value="" disabled>Select Model</option>{models.map((item) => <option value={item.model} key={item.model}>{item.modelLabel}</option>)}</select></label><label>Mode<select value={selected?.modes.some((item) => item.id === config.modeId) ? config.modeId : ""} disabled={!selected} onChange={(event) => onChange({ ...config, provider, model, modeId: event.target.value || undefined })}><option value="">Model Default</option>{(selected?.modes ?? []).map((item) => <option value={item.id} key={item.id}>{item.label}</option>)}</select></label><label>Thinking<select value={selected?.thinkingOptions.some((item) => item.id === config.thinkingOptionId) ? config.thinkingOptionId : ""} disabled={!selected} onChange={(event) => onChange({ ...config, provider, model, thinkingOptionId: event.target.value || undefined })}><option value="">Model Default</option>{(selected?.thinkingOptions ?? []).map((item) => <option value={item.id} key={item.id}>{item.label}</option>)}</select></label></div></article>;
}
