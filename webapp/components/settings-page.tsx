...
function PaseoSetupWizard({ request, onClose }: { request<T = unknown>(url: string, init?: RequestInit): Promise<T>; onClose(): void }) {
  ...
  return <div className="dialog-backdrop" role="presentation"><section className="dialog connection-wizard" role="dialog" aria-modal="true" aria-labelledby="paseo-wizard-title">
    ...
    {step === 1 && <div className="wizard-panel"><div><h3>How should Agent God Mode reach this host?</h3><p>You can add the other connection method later.</p></div><div className="connection-methods"><button type="button" className={transport === "relay" ? "selected" : ""} onClick={() => setTransport("relay")}><span><RadioTower/></span><strong>Paseo Relay</strong><small>Recommended for the web app. Connect securely without setting up a private network.</small><i>{transport === "relay" && <Check/>}</i></button><button type="button" className={transport === "tailscale" ? "selected" : ""} onClick={() => setTransport("tailscale")}><span><Wifi/></span><strong>Direct via Tailscale</strong><small>Connect over your private tailnet when this Agent God Mode installation can reach it.</small><i>{transport === "tailscale" && <Check/>}</i></button></div><footer><button className="primary" onClick={() => setStep(2)}>Continue<ArrowRight/></button></footer></div>}
