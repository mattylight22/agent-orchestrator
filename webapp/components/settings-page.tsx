...
export function SettingsPage() {
  ...
  return <section className="page settings-page"><header className="page-header"><div><span className="eyebrow">Preferences</span><h1>Settings</h1><p>Connections, defaults, appearance, and agent roles.</p></div><div className={`settings-save-state ${saveState}`} role="status" aria-live="polite">{saveState === "saving" ? "Saving…" : saveState === "saved" ? <><Check/>Saved</> : saveState === "error" ? "Save failed" : "Changes save automatically"}</div></header>
    <div className="settings-stack">
      ...
      <SettingsSection id="paseo" icon={<Server/>} title="Paseo hosts" description="Connect each host through Relay, Tailscale, or deploy one in AWS."><PaseoHosts hosts={snapshot.hosts} providerCatalogs={snapshot.providerCatalogs} request={request} onDeployAws={() => setAwsWizardOpen(true)}/></SettingsSection>
      <SettingsSection id="aws" icon={<Cloud/>} title="AWS accounts" description="Deploy and manage always-on Paseo hosts with short-lived, scoped AWS access."><AwsAccountsPanel onAdd={() => setAwsWizardOpen(true)}/></SettingsSection>
      {catalogs.length > 0 && <SettingsSection icon={<Bot/>} title="Agent defaults" description="Choose from providers authenticated on your connected Paseo hosts."><div className="role-grid">{(["planner", "builder", "reviewer"] as AgentRole[]).map((role) => <RoleEditor role={role} key={role} config={settings.globalRoles[role]} catalogs={catalogs} onChange={(config) => update({ globalRoles: { ...settingsRef.current.globalRoles, [role]: config } })}/>)}</div></SettingsSection>}
...
export function PaseoSetupWizard({ request, onClose, onDeployAws }: { request<T = unknown>(url: string, init?: RequestInit): Promise<T>; onClose(): void; onDeployAws?: () => void }) {
  ...
  return <div className="dialog-backdrop" role="presentation"><section className="dialog connection-wizard" role="dialog" aria-modal="true" aria-labelledby="paseo-wizard-title">
    ...
    {step === 1 && <div className="wizard-panel"><div><h3>How should Agent God Mode reach this host?</h3><p>Connect an existing machine or deploy a dedicated host in your AWS account.</p></div><div className="connection-methods"><button type="button" className={transport === "tailscale" ? "selected" : ""} onClick={() => setTransport("tailscale")}><span><Wifi/></span><strong>Direct via Tailscale</strong><small>Your browser connects straight to Paseo through Tailscale on this device. Traffic stays inside your tailnet.</small><i>{transport === "tailscale" && <Check/>}</i></button><button type="button" className={transport === "relay" ? "selected" : ""} onClick={() => setTransport("relay")}><span><RadioTower/></span><strong>Paseo Relay</strong><small>Connect from any device without joining the tailnet. End-to-end encrypted with no inbound ports.</small><i>{transport === "relay" && <Check/>}</i></button>{onDeployAws && <button type="button" onClick={onDeployAws}><span><Cloud/></span><strong>Deploy on AWS</strong><small>Connect an AWS account with OIDC, create an always-on EC2 host, and pair it automatically through Relay.</small><i><ArrowRight/></i></button>}</div><footer><button className="primary" onClick={() => setStep(2)}>Continue<ArrowRight/></button></footer></div>}
