...
export function PlansPage() {
  ...
  return <section className="page"><header className="page-header"><div><span className="eyebrow">Plan library</span><h1>Plans</h1><p>Review, stage, sequence, and launch implementation-ready work.</p></div></header>
    ...
    <div className="plan-grid">{plans.map((plan) => <article className="plan-card" key={plan.id}><header><div className="plan-icon"><GitBranch/></div><div><Link href={`/workstreams/${plan.workstreamId}?tab=plan`}>{plan.title}</Link><small>{plan.repositoryFullName}</small></div><StatusChip value={plan.status}/></header><p>{plan.body.replace(/[#>*`]/g, "").slice(0, 220)}{plan.body.length > 220 ? "…" : ""}</p><div className="plan-meta"><span><StatusChip value={plan.executionState}/></span>{plan.blockedByIds.length > 0 && <span>{plan.blockedByIds.length} prerequisite{plan.blockedByIds.length === 1 ? "" : "s"} pending</span>}</div><footer><select value={plan.status} onChange={(event) => void update(plan.id, event.target.value as PlanStatus)}><option value="product-feature">Product / Feature Plan</option><option value="implementation-ready">Implementation Ready</option><option value="cancelled">Cancelled</option></select><Link className="button" href={`/workstreams/${plan.workstreamId}?tab=plan`}>Open <ArrowRight/></Link>{plan.status === "implementation-ready" && !plan.blockedByIds.length && plan.executionState !== "completed" && <button className="primary" onClick={() => void request(`/api/plans/${plan.id}/actions`, { method: "POST", body: JSON.stringify({ action: "begin" }) })}><Check/>Begin work</button>}</footer></article>)}</div>
