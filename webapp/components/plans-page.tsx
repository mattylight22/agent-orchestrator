...
import { ArrowRight, Check, GitBranch, Maximize2, Search } from "lucide-react";
...
export function PlansPage() {
  ...
  const readerPlan = snapshot.plans.find((plan) => plan.id === readerPlanId);
  ...
  return <section className="page"><header className="page-header"><div><span className="eyebrow">Plan library</span><h1>Plans</h1><p>Review, stage, sequence, and launch implementation-ready work.</p></div></header>
    ...
    {readerPlan && <PlanReaderModal title={readerPlan.title} repository={readerPlan.repositoryFullName} status={readerPlan.status} body={readerPlan.body} workstreamHref={`/app/workstreams/${readerPlan.workstreamId}?tab=plan`} onClose={() => setReaderPlanId(null)}/>}
  </section>;
}
