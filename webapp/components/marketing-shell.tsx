...
export function MarketingShell({ signedIn, children }: { signedIn: boolean; children: React.ReactNode }) {
  return <div className="marketing-root" data-session={signedIn ? "authenticated" : "anonymous"}>
    <header className="marketing-header">
      ...
      <nav aria-label="Public navigation"><Link href="/product">Product</Link><Link href="/security">Security</Link></nav>
    ...
    {children}
    <footer className="marketing-footer"><div><Link className="marketing-wordmark" href="/"><img src={appIcon.src} alt=""/><strong>Agent God Mode</strong></Link><p>Structured orchestration for serious software work.</p></div><nav aria-label="Footer navigation"><Link href="/product">Product</Link><Link href="/security">Security</Link><Link href="/login?next=/app" className="signed-out-only">Sign in</Link><Link href="/app" className="signed-in-only">Open app</Link></nav><small>Built for GitHub-backed workstreams running through Paseo.</small></footer>
  </div>;
}
