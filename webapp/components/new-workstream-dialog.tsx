...
export function NewWorkstreamDialog({ open, onClose }: { open: boolean; onClose(): void }) {
  ...
  return <div className="dialog-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><form className="dialog" onSubmit={submit}>
    ...
    {!snapshot.hosts.length && <div className="banner warning">Connect an Agent Instance in Settings before creating a workstream.</div>}
    ...
    <div className="field-grid"><label>Workstream Name<input value={name} onChange={(event) => setName(event.target.value)} placeholder="Improve onboarding flow" required /></label><label>Agent Instance<select value={hostId} onChange={(event) => setHostId(event.target.value)} required><option value="">Select Instance…</option>{snapshot.hosts.map((host) => <option value={host.id} key={host.id}>{host.name}{repository?.hostAvailability.some((item) => item.hostId === host.id) ? " · ready" : " · will register repo"}</option>)}</select></label></div>
