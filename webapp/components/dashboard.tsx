...
export function Dashboard() {
  ...
  return <section className="page dashboard-page">
    ...
    <div className="data-panel">
      ...
      <div className="table-wrap"><table><thead><tr><th>Workstream</th><th>Repository</th><th>Branch</th><th>Status</th><th>Phase</th><th>Agent</th><th>Updated</th><th>PR</th></tr></thead><tbody>{rows.map((item) => <tr key={item.id}><td><Link className="row-title" href={`/workstreams/${item.id}`}>{item.name}</Link></td><td>{item.repositoryFullName}</td><td><code title={item.branchName}>{item.branchName}</code></td><td><StatusChip value={item.status}/></td><td><StatusChip value={item.phase}/></td><td><span className={`agent-state ${item.agentState}`}><i />{item.agentState}</span></td><td title={new Date(item.updatedAt).toLocaleString()}>{relative(item.updatedAt)}</td><td>{item.prUrl ? <a className="external" href={item.prUrl} target="_blank" rel="noreferrer">#{item.prNumber}<ArrowUpRight /></a> : "—"}</td></tr>)}</tbody></table></div>
