...
export function AwsAccountWizard({ onClose }: { onClose(): void }) {
  ...
  const [deploymentName, setDeploymentName] = useState("Paseo host");
  ...
  return <div className="dialog-backdrop" role="presentation"><section className="dialog aws-account-wizard" role="dialog" aria-modal="true" aria-labelledby="aws-wizard-title">
    <header><div><span className="eyebrow">Credentialless AWS</span><h2 id="aws-wizard-title">Deploy a Paseo host</h2></div><button className="icon-button" aria-label="Close AWS setup" onClick={onClose}><X/></button></header>
...
export function AwsAccountsPanel({ onAdd }: { onAdd(): void }) {
  ...
  return <div className="aws-accounts-panel">
    ...
    {!accounts.length && <div className="empty-connection"><Cloud/><div><strong>No AWS accounts connected</strong><small>Connect with short-lived credentials to deploy and manage a Paseo host without AWS access keys.</small></div></div>}
