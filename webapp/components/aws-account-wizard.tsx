...
interface SetupPayload { token: string; stackName: string; roleName: string; launchUrl: string; outputsUrl: string; trustPolicy: string; permissionsPolicy: string }
...
export function AwsAccountWizard({ onClose }: { onClose(): void }) {
  ...
  const initialAccount = accounts.find((item) => item.state === "connected");
  const [stage, setStage] = useState<Stage>(accounts.length ? "accounts" : "create");
  const [connectionId, setConnectionId] = useState(initialAccount?.id ?? "");
  ...
  const [deploymentName, setDeploymentName] = useState(initialAccount?.accountId ? `Agent Instance - ${initialAccount.accountId}` : "Agent Instance");
  ...
  useEffect(() => {
    if (stage === "deploy" && selectedAccount?.accountId) setDeploymentName(`Agent Instance - ${selectedAccount.accountId}`);
  }, [selectedAccount?.accountId, stage]);
  ...
  async function retryDeployment() {
    ...
    try { await request(`/api/aws/deployments/${deployment.id}/actions`, { method: "POST", body: JSON.stringify({ action: "retry" }) }); }
    catch (reason) { setError(reason instanceof Error ? reason.message : "Could not resume AWS provisioning"); }
    finally { setBusy(""); }
  }
  ...
  return <div className="dialog-backdrop" role="presentation"><section className="dialog aws-account-wizard" role="dialog" aria-modal="true" aria-labelledby="aws-wizard-title">
    ...
    {stage === "access" && setup && <form className="wizard-panel aws-access-panel" onSubmit={verify}>
      <div><h3>Authorize Agent God Mode in AWS</h3><p>The recommended stack creates three restricted IAM roles and the EC2 instance profile needed for your Agent Instance.</p></div>
      <a className="primary button aws-launch-access" href={setup.launchUrl} target="_blank" rel="noreferrer">Create AWS Access<ExternalLink/></a>
      <div className="aws-output-guide">
        <header><span>After the Stack Finishes</span><a className="button" href={setup.outputsUrl} target="_blank" rel="noreferrer">Open Stack Outputs<ExternalLink/></a></header>
        <ol>
          <li><span>1</span><p>Wait until stack <code>{setup.stackName}</code> shows <strong>CREATE_COMPLETE</strong>.</p></li>
          <li><span>2</span><p>Open its <strong>Outputs</strong> tab and find the row whose key is <code>ConnectionRoleArn</code>.</p></li>
          <li><span>3</span><p>Copy that row’s <strong>Value</strong> and paste it below.</p></li>
        </ol>
      </div>
      <label>Paste the ConnectionRoleArn Value<input value={roleArn} onChange={(event) => setRoleArn(event.target.value)} placeholder={`arn:aws:iam::123456789012:role/${setup.roleName}`} spellCheck={false} required/><small>Use <code>ConnectionRoleArn</code>—not <code>ExecutionRoleArn</code> or <code>InstanceProfileName</code>.</small></label>
      <button type="button" className="text-button" onClick={() => setManual((value) => !value)}>{manual ? "Hide Manual IAM Setup" : "Set Up AWS Access Manually"}</button>
      {manual && <div className="manual-iam"><p>Create an IAM role named <code>{setup.roleName}</code>, choose <strong>Custom Trust Policy</strong>, and paste the first document. Then add an inline permissions policy with the second document.</p><JsonCopyBox title="Trust Policy" value={setup.trustPolicy}/><JsonCopyBox title="Permissions Policy" value={setup.permissionsPolicy}/><div className="banner warning">Automatic deployment also needs the restricted CloudFormation and EC2 roles. The complete template creates all three roles.</div><a href="/aws/agent-god-mode-aws-access.yaml" download>Download Complete IAM Template</a></div>}
      <footer><button type="button" className="button" onClick={() => setStage(accounts.length > 1 ? "accounts" : "create")}><ArrowLeft/>Back</button><button className="primary" disabled={busy === "verify"}>{busy === "verify" ? <LoaderCircle className="spinning"/> : null}Verify Access<ArrowRight/></button></footer>
    </form>}
...
export function DeploymentProgress({ deployment }: { deployment?: AwsPaseoDeployment }) {
  ...
  const failureText = deployment?.failureDetail ?? "";
